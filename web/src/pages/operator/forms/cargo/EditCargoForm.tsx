...
type EditCargoProps = {
  ...
  onSubmit?: (data: { id: number; title: string; typeId: number; weight: number; volume: number; vesselId: number }) => void
  onCancel?: () => void
}

export default function EditCargo({ cargo, onSubmit, onCancel }: EditCargoProps) {
  ...
  useEffect(() => {
    ...
    setTypeId(cargo.typeId)
    setWeight(cargo.weight)
    setVolume(cargo.volume)
    setVesselId(cargo.vesselId)
  }, [cargo])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    else alert(`Обновляем груз: ${JSON.stringify(data)}`)
  }

  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <h2 className="text-xl font-bold mb-2">Редактировать груз</h2>
